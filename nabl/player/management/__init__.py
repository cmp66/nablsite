...
class PlayerManager:
    ...
    def get_player_info(self, lastname, firstname=None, target_year=None):
        ...
        player_ids = playerid_lookup(lastname) if firstname is None else playerid_lookup(lastname, firstname)
        if player_ids.empty:
            player_ids = playerid_lookup(self.get_modified_lastname(lastname)) if firstname is None else playerid_lookup(self.get_modified_lastname(lastname), firstname)
        ...
        for index, row in player_ids.iterrows():
            ...
            if target_year is not None and (target_year < mlb_played_first or target_year > mlb_played_last):
                continue
    ...
    def create_player_from_info(self, player_info, max_endyear=None):
        ...
        player.displayname = f'{unidecode(player.firstname)} {unidecode(player.lastname)}'
        player.startyear = player_info['mlb_played_first']

        if max_endyear is not None and player_info['mlb_played_last'] > int(max_endyear):
        ...
        if player_info:
            for possible_player in player_info:
                if possible_player['key_bbref'] == player.bbrefid:
                    result = {}
                    result['full_bbref'] = f'https://www.baseball-reference.com/players/{possible_player["key_bbref"][0]}/{possible_player["key_bbref"]}.shtml'
                    result['start_year'] = possible_player['mlb_played_first']
                    result['end_year'] = possible_player['mlb_played_last']
                    result['first_name'] = string.capwords(possible_player['name_first'])
                    ...
                    result['position'] = self.get_player_position(possible_player['key_lahman'])
    ...
    def search_for_player_by_fullname(self, fullname, target_year=2000):
        ...
        players = Players.objects.filter(displayname=fullname).filter(start_year__lte=target_year).filter(end_year__gte=target_year)
        ...
        if not player_data:
            player_data = self.check_for_player(player, player.displayname.split(maxsplit=3)[-1])

        if not player_data:
            player_data = self.check_for_player(player, f'{player.displayname.split(maxsplit=2)[-2]} {player.displayname.split(maxsplit=2)[-1]}')
