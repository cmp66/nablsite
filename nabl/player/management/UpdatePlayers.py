...
for player in players:
    ...
    if not player_data:
        player_data = manager.check_for_player_sync(player, player.displayname.split(maxsplit=3)[-1])

    if not player_data:
        player_data = manager.check_for_player_sync(player, f'{player.displayname.split(maxsplit=2)[-2]} {player.displayname.split(maxsplit=2)[-1]}')

    if not player_data:
        print (f'No player found for {player.firstname} {player.lastname} {player.bbreflink}')
    else:
